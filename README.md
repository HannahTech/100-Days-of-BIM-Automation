# 100-Days-of-BIM-Automation

🏗️ 100 Days of Snowdon: AEC Automation Suite
This repository tracks my 100-day journey of automating a high-complexity architectural project (Snowdon Towers) from the ground up. I am replacing manual modeling tasks with Python-driven systems and Revit API tools.

Tech Stack: * Languages: Python, SQL
Tools: Revit API, pyRevit, Dynamo, Navisworks Manage, Power BI
Methodology: ISO 19650 Standards, BIM Level 2

📈 Project Milestones:

[ ] Days 1-25: Foundation & Data (Automated Levels, Rooms, & Parameters)
[ ] Days 26-50: Coordination (Clash-to-Action Python Pipeline)
[ ] Days 51-75: Analytics (Model Health Dashboarding)
[ ] Days 76-100: Deployment (Beta Testing B2B Efficiency Tools)
